...
export const a08Theory: TheorySection[] = [
  /* ─────────── TẦNG 1 ─────────── */
  {
    ...
    paras: [
      bi(
        ...
        'Pipeline nào cũng phải trả lời một câu: dữ liệu mới về thì nó vào bảng bằng đường nào? Nếu chuyện của hôm qua không bao giờ đổi thì bạn cứ append mãi rồi về nhà là xong.',
      ),
      bi(
        ...
        'Nhưng feed này thì gửi kèm cả bản sửa, và gần như feed thật nào cũng vậy. File hôm nay lặng lẽ chứa phiên bản mới của những đơn hàng từ cả tuần trước: vẫn order_id đó, status đã tiến thêm một bước, có khi tổng tiền đổi, và một updated_at mới. Chừng 1,5% mỗi file là loại đó.',
      ),
      bi(
        ...
        'Chọn nhầm chiến lược load thì warehouse hoặc đếm trùng đơn hàng, hoặc lặng lẽ hiển thị status đã cũ. Cả hai kiểu đều im lặng: chẳng lỗi nào báo, chỉ là con số trên dashboard tình cờ sai.',
      ),
    ],
    checks: [
      {
        q: bi(
          ...
          'Một bản sửa về, mà updated_at của nó lại cũ hơn phiên bản bạn đang có. Xử lý thế nào?',
        ),
        a: bi(
          ...
          'Nó phải thua. Luật là bản có updated_at mới nhất thắng, chứ không phải bản về sau thắng. updated_at mới là thứ quyết định phiên bản nào đang có hiệu lực, còn thứ tự file về thì chẳng nói lên điều gì. Feed này gửi những dòng như vậy thật, và Task 7 sẽ cho bạn gặp một cái tận mắt.',
        ),
      },
    ],
  },
  ...
  /* ─────────── TẦNG 2 ─────────── */
  {
    ...
    paras: [
      bi(
        ...
        'Đây không phải bốn phương án trong vô số phương án. Ba cái đầu chính là ba câu trả lời của ngành, và mọi công cụ bạn từng gặp, kể cả dbt, chỉ là lớp vỏ bọc quanh một trong ba cái đó.',
      ),
    ],
    alternatives: [
      {
        ...
        appeal: bi(
          ...
          'Cách ghi rẻ nhất có thể: không tra cứu gì, không xoá gì. Mà với một event log kiểu click hay lượt xem trang thì nó đúng thật.',
        ),
        breaks: bi(
          ...
          'Nhưng ở đây thì không đúng. Feed này gửi bản sửa, nên chỉ append thôi thì bạn có nhiều phiên bản của cùng một đơn mà không có luật nào chọn ra một. Mỗi query phía sau sẽ phải tự nghĩ cách chọn, và chúng sẽ chẳng nghĩ ra giống nhau đâu.',
        ),
      },
      {
        ...
        appeal: bi(
          ...
          'Không update, không xoá: mọi dòng từ mọi file đổ vào một bảng cứ lớn dần, rồi một view dùng window function chọn ra bản thắng cho từng order_id. Ghi thì rẻ hết mức, mà bạn giữ được toàn bộ lịch sử của mọi bản sửa, coi như có sẵn audit trail mà không tốn gì.',
        ),
        breaks: bi(
          ...
          'Cái giá chuyển sang phía người đọc: query nào cũng phải trả tiền cho phép dedupe. Ở scale full là chừng một giây mỗi lần đọc, mãi mãi, với tất cả mọi người. Các event log và tầng bronze vận hành đúng như vậy, và đây là lựa chọn tốt — miễn là bạn biết ai đang trả cái giá đó.',
        ),
      },
      {
        ...
        appeal: bi(
          ...
          'Upsert là ghép của update với insert: có key rồi thì update, chưa có thì insert, gói trong một câu lệnh. Bảng luôn giữ đúng một dòng đang có hiệu lực cho mỗi đơn, nên đọc vừa rẻ vừa đơn giản. Đây là khuôn kinh điển của warehouse cho những thực thể còn thay đổi.',
        ),
        breaks: bi(
          ...
          'Lịch sử thì mất sạch: bản cũ bị ghi đè, và về sau không ai hỏi được câu "chúng ta biết đơn này bị hoàn tiền từ lúc nào?". Ngoài ra nó cần loại kho sửa được tại chỗ, tức là một database chứ không phải một thư mục đầy file. Chĩa nó vào Parquet lake thì đơn giản là chạy không được.',
        ),
      },
      {
        name: bi('Strategy C — partition refresh (delete+insert by date)', 'Chiến lược C — refresh cả partition, tức xoá rồi insert theo ngày'),
        appeal: bi(
        ...
        breaks: bi(
          ...
          'Cái giá là I/O hàng loạt: ghi lại chừng 8 ngày dữ liệu chỉ để nạp 1 ngày. Nhưng đây là chiến lược duy nhất chạy được khi kho của bạn là những file bất biến — bạn không sửa được một dòng bên trong file Parquet, chỉ ghi lại cả file thôi. Vì thế nó thắng ở các file lake.',
        ),
      },
    ],
    checks: [
      {
        ...
        a: bi(
          ...
          'Từ contract ở A06: điều khoản late_corrections cam kết bản sửa về trễ nhiều nhất 7 ngày. Cửa sổ refresh phải phủ hết mọi partition mà một phiên bản của đơn hàng đang về có thể nằm trong đó, và chính contract làm cho cửa sổ ấy có căn cứ. Không có cam kết bằng văn bản thì bạn chỉ đang đoán.',
        ),
      },
    ],
  },
  ...
  /* ─────────── TẦNG 3 ─────────── */
  {
    ...
    paras: [
      bi(
        ...
        'Cả ba chiến lược, chạy trên cùng bộ file, phải cho ra trạng thái hiện hành giống hệt nhau. Khác nhau thì ít nhất một cái sai. Chứng minh được chúng khớp nhau mới là trọng tâm của bài này, chứ không phải dựng xong một chiến lược nào cả.',
      ),
      bi(
        ...
        'Luật nghiệp vụ nằm dưới cả ba gói trong một câu: với mỗi order_id, phiên bản có updated_at mới nhất thắng. Cột updated_at chính là thứ có quyền quyết định bản nào đang có hiệu lực. Không phải thứ tự file, không phải lúc nó về, cũng không phải dòng bạn nhìn thấy sau cùng.',
      ),
      bi(
        ...
        'Với lại hai cái grain ở đây không phải cùng một bảng mặc áo khác nhau. Log thì một dòng bằng một phiên bản đơn hàng đã được giao; view thì một dòng bằng một đơn hàng, ở bản mới nhất của nó. Chính vì vậy chúng là hai đối tượng chứ không phải một, và cũng vì vậy mà cùng một cửa sổ cho ra 577.673 dòng ở cái đầu và 569.639 ở cái sau.',
      ),
    ],
    checks: [
      {
        q: bi(
          ...
          'Hai chiến lược cùng trả về 569.639 dòng. Vậy chúng đã khớp nhau chưa?',
        ),
        a: bi(
          ...
          'Chưa chứng minh được gì cả. Hai bảng bằng nhau về số dòng vẫn có thể lệch nhau hàng nghìn dòng. Task 6 bắt bạn chạy một cửa sổ sai có chủ đích, số dòng ra 569.640 — lệch đúng một, con số chẳng ai để ý — trong khi thật ra có 5.824 dòng sai. Đếm không phải là reconcile; phải diff nội dung, và diff cả hai chiều.',
        ),
      },
    ],
  },
  ...
  /* ─────────── TẦNG 4 ─────────── */
  {
    ...
    heading: bi(
      ...
      'Cái gì làm cho từng chiến lược chạy đúng',
    ),
    paras: [
      bi(
        ...
        'Chiến lược A dựa vào đúng một window function: đánh số các phiên bản của mỗi đơn, updated_at mới nhất đứng trước, rồi giữ dòng số 1. Khoá sắp xếp thứ hai đóng vai tie-breaker: nếu hai phiên bản có cùng updated_at thì luật file về trước thắng khiến lựa chọn trở nên deterministic. Đây không phải chuyện giả định — ở scale full, cửa sổ này có đúng một thế hoà thật, với hai tổng tiền khác nhau.',
      ),
      bi(
        ...
        'Chiến lược B cần hai luật, mà cả hai đều dễ quên. Luật một: phía nguồn mỗi key chỉ được một dòng. Feed này có bản trùng ngay trong file, mà nếu nguồn có hai dòng cho cùng một order_id thì MERGE của DuckDB không báo lỗi — nó lặng lẽ chọn bừa một dòng. Nên phải dedupe từng ngày ở staging trước. Luật hai: chặn phép update bằng điều kiện s.updated_at > t.updated_at, không thì một bản sửa cũ sẽ đè lên dòng mới hơn.',
      ),
      bi(
        ...
        'Chiến lược C giấu cái bẫy của nó ngay trong hình dạng của một câu query: phải dedupe trên toàn bộ log tính tới ngày D trước, rồi mới giữ lại những bản thắng nằm trong cửa sổ. Chọn bản thắng trên toàn cục, đặt chúng vào chỗ cục bộ. Cái "tối ưu" nghe rất hợp lý là lọc về cửa sổ trước rồi mới dedupe thì sai một cách rất tinh vi: bản thắng thật của một đơn có thể nằm ngoài cửa sổ, và dedupe cục bộ sẽ làm sống lại một phiên bản đã cũ.',
      ),
      bi(
        ...
        'Chỗ khó chịu là ở scale small hai cách viết cho kết quả giống hệt nhau, còn ở full thì cách sai để lại 7 đơn hàng bị trùng. Viết code trên small thì được, nhưng đừng bao giờ nghiệm thu trên small.',
      ),
    ],
    checks: [
      {
        ...
        a: bi(
          ...
          'Cái ý nghĩ khớp key thì update nghe rất hiển nhiên mà lại sai. Trên cửa sổ này, một lệnh MERGE không có điều kiện chặn sẽ để lại chừng 1.860 dòng cũ trong im lặng — thành ra file về sau thắng chứ không phải bản mới nhất thắng. Số dòng không đổi, nên chỉ diff nội dung mới bắt được.',
        ),
      },
      {
        q: bi(
        ...
        a: bi(
          ...
          'Vì một dòng có ngày NULL thì không bao giờ khớp điều kiện WHERE order_date BETWEEN — không bị xoá, cũng không được insert lại. Chiến lược C sẽ lặng lẽ đánh mất nó vĩnh viễn. Chừng 0,02% số dòng có timestamp bất khả thi, làm sạch xong thì thành NULL, nên order_date cần một COALESCE lùi về ngày của file.',
        ),
      },
    ],
  },
  ...
  /* ─────────── TẦNG 5 ─────────── */
  {
    ...
    heading: bi(
      ...
      'Chuyện ghi nhiều gấp mấy lần, cái bẫy khi chạy lại, và cuối cùng nên chọn gì',
    ),
    paras: [
      bi(
        ...
        'Chuyện ghi ra nhiều gấp mấy lần lượng nhận vào không phải lỗi, đó là điều kiện của cuộc trao đổi. Chiến lược C nhận 577 nghìn dòng và ghi ra chừng 2,98 triệu, tức gấp năm lần. Ở scale full là ghi 60 triệu dòng để giao 11,6 triệu. Bạn trả bằng I/O hàng loạt, đổi lại được sự đơn giản và chạy được trên file.',
      ),
      bi(
        ...
        'Có một cái bẫy đáng ghi vào journal: chạy lại ngày mới nhất thì idempotent hoàn hảo, nhưng chạy lại một ngày D cũ hơn sẽ tua cửa sổ từ D trừ 7 tới D về đúng trạng thái của thời điểm D — mọi bản sửa về sau ngày D biến mất, cho tới khi bạn chạy lại từ D cộng 1 trở đi. Chạy lại một lần refresh thực chất là một lần backfill thu nhỏ: khởi động lại từ D rồi cuốn tới hiện tại.',
      ),
      bi(
      ...
      bi(
        ...
        'Vậy rốt cuộc nên chọn gì? Câu trả lời của lab: giữ append log làm nguồn bền vững, vì chỉ nó trả lời được câu chúng ta biết chuyện này từ lúc nào, rồi phục vụ bảng core.orders bằng cách refresh partition từ log đó. Cách này cho bạn cả lịch sử lẫn việc đọc rẻ, đổi lại phải chịu chi phí ghi lại hàng loạt.',
      ),
      bi(
        ...
        'Câu chuyện chưa dừng ở đây. A09 sẽ lo chuyện người đọc nhìn thấy gì trong lúc refresh đang chạy — hiện giờ vẫn có một khoảng thời gian bảng bị ghi lại dở dang. A11 sẽ chạy lại pipeline theo ngày này trên toàn bộ lịch sử. Còn A15 cho thấy dbt đóng gói đúng chiến lược này thành ba dòng cấu hình: cái incremental strategy kiểu delete rồi insert của nó chính là chiến lược C.',
      ),
    ],
    checks: [
      {
        q: bi(
          ...
          'Vòng lặp refresh của bạn dùng cửa sổ D tới D thay vì D trừ 7 tới D. Số dòng ra 569.640 thay vì 569.639. Vậy coi là gần đúng được không?',
        ),
        a: bi(
          ...
          'Không — có 5.824 dòng sai. Mọi đơn hàng mà bản đang có hiệu lực là một bản sửa đều bị kẹt ở bản gốc đã cũ, vì bản sửa rơi vào một partition cũ hơn mà cửa sổ hẹp không hề chạm tới. Số dòng gần như không nhúc nhích, và chính điều đó khiến bug này sống sót được tới môi trường thật.',
        ),
      },
    ],
  },
...
export const a08Terms: Term[] = [
  {
    ...
    means: bi(
      ...
      'Phiên bản mới của một đơn hàng đã xuất hiện ở ngày trước đó: vẫn order_id đó, status tiến thêm một bước, updated_at mới. Chiếm chừng 1,5% mỗi file. Contract cho phép trễ nhiều nhất 7 ngày.',
    ),
    source: {
  ...
  },
  {
    ...
    gloss: 'cột quyết định bản nào đang có hiệu lực',
    means: bi(
      ...
      'Cột quyết định bản nào của một dòng đang có hiệu lực. Ở đây là updated_at, chứ không phải thứ tự file hay lúc nó về. Một bản sửa có updated_at cũ hơn thì phải thua, dù nó về sau.',
    ),
    source: {
      name: 'dbt — Snapshots',
  ...
  },
  {
    ...
    means: bi(
      ...
      'Không update, không xoá — mọi dòng từ mọi file rơi vào một bảng cứ lớn dần. Ghi rẻ nhất, giữ đủ lịch sử, nhưng lần đọc nào cũng phải trả tiền cho phép dedupe.',
    ),
    source: {
  ...
  },
  {
    ...
    means: bi(
      ...
      'Một view chọn ra bản thắng cho từng key bằng row_number() OVER (PARTITION BY key ORDER BY phiên bản DESC). Nó dời chi phí từ lúc ghi sang lúc đọc, và ai đọc cũng phải chịu, mãi mãi.',
    ),
    source: {
  ...
  },
  {
    ...
    means: bi(
      ...
      'Một câu lệnh vừa update khi key đã có, vừa insert khi chưa có. Cần nguồn đã dedupe sẵn, cần điều kiện chặn theo phiên bản, và cần loại kho sửa được tại chỗ.',
    ),
    source: {
  ...
  },
  {
    ...
    means: bi(
      ...
      'Xoá một cửa sổ ngày rồi insert lại từ đầu. Ghi lại chừng 8 ngày để nạp 1 ngày, nhưng đây là cách cập nhật duy nhất mà file bất biến cho phép — bạn không sửa được một dòng bên trong file Parquet.',
    ),
    source: {
  ...
  },
  {
    ...
    means: bi(
      ...
      'Cam kết trong contract rằng bản sửa về trễ nhiều nhất N ngày. Chính nó làm cho cửa sổ refresh có căn cứ — không có cam kết bằng văn bản thì bạn chỉ đang đoán phải ghi lại lùi về bao xa.',
    ),
    source: {
      name: 'Apache Flink — event time',
      url: 'https://nightlies.apache.org/flink/flink-docs-stable/docs/concepts/time/',
  ...
  },
  {
    ...
    means: bi(
      ...
      'Điều kiện _data_date nhỏ hơn hoặc bằng D, để tái hiện lịch sử cho trung thực: vào ngày D thì pipeline mới chỉ thấy các file tính tới D. Pipeline chạy thật thì nó tự đúng; mô phỏng quá khứ thì bạn phải tự viết ra.',
    ),
    source: {
      name: 'dbt — Snapshots',
      url: 'https://docs.getdbt.com/docs/build/snapshots',
